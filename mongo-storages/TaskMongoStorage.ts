...
export class TaskMongoStorage implements ITaskStorage {
    ...
    public create(createTaskDTO: CreateTaskDTO): Promise<Task> {
        return new Promise((resolve, reject) => {
            this._collection.create(createTaskDTO).then((response: ITaskModel) => {
    ...
    public getById(getByIdDTO: GetByIdDTO): Promise<Task | null> {
        return new Promise((resolve, reject) => {
            this._collection.findById(getByIdDTO.id).then((response: ITaskModel | null) => {
                if (null != response) {
                    const task: Task = response
                    resolve(task)
                }
    ...
    public list(listTasksDTO: ListTasksDTO): Promise<Task[]> {
        return new Promise((resolve, reject) => {
            ...
            this._collection.find(query, null, options).then((response: ITaskModel[]) => {
                if (null != response) {
                    const tasks: Task[] = []
